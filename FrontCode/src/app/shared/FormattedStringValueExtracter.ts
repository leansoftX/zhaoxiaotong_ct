    ...
    Tokenize(format: string, includeBracketsForDynamicValues: boolean = false): FormatStringToken[] {
        ...
        for (let i = 0; i < format.length; i++) {
            ...
            switch (c) {
                case '{':
                    if (inDynamicValue) {
                        throw new Error('Incorrect syntax at char ' + i + '! which can not contain nested dynamic value expression!');
                    }
                    inDynamicValue = true;
...
export class FormattedStringValueExtracter {
    ...
    IsMatch(str: string, format: string): string[] {
        ...
        const values = [];
        for (const matche of result.Matches) {
            values.push(matche.value);
        }
        return values;
    }
}
